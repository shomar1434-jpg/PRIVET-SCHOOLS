(function(g){
  ...
  async function userAdmin(action,payload={}){const ctx=await requireContext(['manager']);return invoke('private-school-user-admin',{action,schoolId:ctx.schoolId,...payload})}
  async function inspectSchoolRegistration(schoolId,registrationCode){return invoke('private-school-registration',{action:'inspect',schoolId:clean(schoolId),registrationCode:clean(registrationCode)},{allowAnonymous:true})}
  async function registerSchoolUser(payload){return invoke('private-school-registration',{action:'register',schoolId:clean(payload.schoolId),registrationCode:clean(payload.registrationCode),email:clean(payload.email),fullName:clean(payload.fullName),password:String(payload.password||''),role:clean(payload.role)},{allowAnonymous:true})}
  ...
  g.PrivateSchoolBridge=Object.freeze({getClient,authScope,schoolLoginPath,login,logout,establishContext,requireContext,privateContext,roleLanding,inspectInvite,acceptInvite,owner,manager,staff,workflows,compliance,performance,messages,tasks,directory,files,uploadModuleFile,template,outputs,selfEvaluationOutput,registrationLink,userAdmin,inspectSchoolRegistration,registerSchoolUser,provisionPrivateSchool,privateSchoolOverview,applyCompatibility,clearPrivateCompat,ROLE_MAP});
})(window);
