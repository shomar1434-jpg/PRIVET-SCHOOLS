...
Deno.serve(async(req)=>{if(req.method==='OPTIONS')return new Response('ok',{headers:cors});try{
 ...
 if(['get_portal','overview','context'].includes(action)){
   ...
   return json({ok:true,school:school.data,owners:ownerList,canManageOwners,managers,manager:activeManager,pendingManagers,invitations:(invs.data||[]).map((x:any)=>({...x,invitee_email:x.email,invite_role:x.role})),managerLoginPath:activeManager?`private-manager-login.html?schoolId=${encodeURIComponent(sid)}`:''});
 }
 if(action==='add_owner'){
