...
(function(){
  ...
  async function render(){ensureView(); renderFolders(); const list=document.getElementById('sectionRepoListView'); document.getElementById('sectionRepoPreview').style.display='none'; list.style.display='grid'; list.innerHTML='<div style="padding:18px;color:#64748b;font-weight:900">جاري تحميل الملفات...</div>'; const allRows=await all(); const rows=allRows.filter(r=>(r.folder||'عام')===activeFolder).sort((a,b)=>new Date(b.createdAt)-new Date(a.createdAt)); if(!rows.length){list.innerHTML=`<div style="padding:28px;text-align:center;border:2px dashed #cbd5e1;border-radius:18px;color:#94a3b8;font-weight:900">لا توجد ملفات في مجلد ${esc(activeFolder)}.</div><button type="button" onclick="document.getElementById('sectionRepoUploadHidden').click()" style="margin-top:15px;width:100%;padding:14px;border:2px dashed #1e7b78;background:#f0fdfa;color:#1e7b78;cursor:pointer;border-radius:18px;font-weight:bold">+ إضافة ملفات إلى هذا المجلد</button>`;return;} list.innerHTML=rows.map(r=>{const k=r.kind||kind(r),id=JSON.stringify(String(r.id)); return `<div class="repo-file"><div><div class="repo-file-title">${icon(k)} — ${esc(r.name)}</div><div class="repo-file-meta">${new Date(r.createdAt).toLocaleString('ar-SA')} — ${fmt(r.size)} — مجلد: ${esc(r.folder||'عام')}</div></div><div style="display:flex;gap:7px;flex-wrap:wrap"><button type="button" class="repo-btn repo-blue" onclick='window.editSectionRepoSafe(${id})'>تعديل</button><button type="button" class="repo-btn repo-add" onclick='window.previewSectionRepoSafe(${id})'>معاينة</button><button type="button" class="repo-btn repo-purple" onclick='window.printSectionRepoSafe(${id})'>طباعة</button><button type="button" class="repo-btn repo-del" onclick='window.deleteSectionRepoSafe(${id})'>حذف</button></div></div>`;}).join('')+`<button type="button" onclick="document.getElementById('sectionRepoUploadHidden').click()" style="margin-top:15px;width:100%;padding:14px;border:2px dashed #1e7b78;background:#f0fdfa;color:#1e7b78;cursor:pointer;border-radius:18px;font-weight:bold">+ إضافة ملفات إلى هذا المجلد</button>`;}
